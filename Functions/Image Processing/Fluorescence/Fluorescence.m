function [ atomNumber, sizeX, sizeY, centreX, centreY] = Fluorescence( atomImage, laserImage, ROI, varargin )
...
exposureTime = 450*1e-6;

% ScatteringRate
lambda = 780*1e-9;
gamma = 2*pi*6.065*1e6; % Hz
detuning=0*1e6;
s = 1; %s is the Saturation parameter I/Isat, Isat = 1.67 mW/cm^2
...
pixelSize = 16*1e-6;
demagnification = 140/80;
sizeX = x1(4)*(2*sqrt(2*log(2)))*pixelSize*demagnification*1000;
sizeY = x2(4)*(2*sqrt(2*log(2)))*pixelSize*demagnification*1000;
...
if or(centreX>max(ROIx),centreX<min(ROIx)) && or(centreY>max(ROIy),centreY<min(ROIy));
