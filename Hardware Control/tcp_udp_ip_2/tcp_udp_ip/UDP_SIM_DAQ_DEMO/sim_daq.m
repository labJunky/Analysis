...
nimage=image(:,1:24);
imageLen = size(nimage);
DATASIZE = imageLen(1)*imageLen(2);

%Setup UDP socket for reciving packets to port RECVPORT
udpsock=pnet('udpsocket',RECVPORT);
pnet(udpsock,'setwritetimeout',2);
pnet(udpsock,'setreadtimeout',2);
    ...
    if len>0, % Packet contain data => Check message
        ...
        for packnr=1:58,
            if packnr==1, 
                ...
                nimage = image(:,(1+24*(packnr-1)):(24*(packnr)));
            end
...
%            pnet(udpsock,'write',sin_wave_data+uint16(packnr*200)); % Send sin wave + DC level as data
            pnet(udpsock,'write',nimage);
            pnet(udpsock,'writepacket',REMOTEIP,REMOTEPORT);
           % fprintf(' %d,',packnr);
            pause(0.005); % You need a very small pause between each packet.
