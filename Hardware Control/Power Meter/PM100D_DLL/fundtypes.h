...
see http://www.codeguru.com/forum/showthread.php?t=323835 */
	...
	namespace LVFloats {
		inline bool Equal64(const float64 &f1, const float64 &f2, FloatComparisonMethod method= FloatComparisonExact) {
			...
			#if Mac && 0	// this prevents a warning on the Mac (but comparing 0.0 to -0.0 reports false, whereas using == reports true)
				const int32	*i1= reinterpret_cast<const int32*>(&f1);
				const int32	*i2= reinterpret_cast<const int32*>(&f2);
