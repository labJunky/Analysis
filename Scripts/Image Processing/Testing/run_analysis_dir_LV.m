...
for k=153:nrOfFiles
    ...
    bg1NameD = dir([directoryBI, '*', objfiles(k).name(1:14),'  BG.tif']);
   ...
   % bg2Exists = exist([directoryBI,nameBG2], 'file')==2;
    ...
    if allExist
       ...
       % [atomNumbers(k), imageArray, odMax(k), sizeX(k), sizeY(k)] = Absorption_normalising( atoms, laser, bg1, bg2, ROI );
         [atomNumbers(k), odMax(k), sizeX(k), sizeY(k)]=Absorption_normalisingLV( atoms, laser, bg1, ROI, imagePath);
...
size(sizes(153:164))
size(times)
[x,temp, resnorm] = FitTemperature(times, sizeX(153:164), name, 'test_temp.fig');
